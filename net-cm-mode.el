...
(setq preview-list-songs nil)
(setq current-playlist nil)

(defun like-this-song()
  (interactive)
  (if current-playlist (progn
			 (setq url-request-data  (format "trackId=%d&like=true&time=0" (cdr (assoc 'id(elt current-playlist current-index)))))
			 (if (= (cdr (assoc 'code (json-loads (visit-163 "http://music.163.com/api/song/like" "POST")))) 502)(progn
															       (setq url-request-data  (format "trackId=%d&like=false&time=0" (cdr (assoc 'id(elt current-playlist current-index)))))
...
(defun fav-list()
  ...
  (if current-playlist-id
      (progn
	(if (= (cdr (assoc 'code (json-loads (visit-163 (format "http://music.163.com/api/playlist/subscribe/?id=%d" current-playlist-id))))) 501)
	    (progn
	      (setq url-request-data  (format "trackId=%d&like=false&time=0" (cdr (assoc 'id(elt current-playlist current-index)))))
...
(defun render ()
  (if (string-match net-cm-buffer-name (buffer-name)) (progn
    ...
    (let ((i 0)
      ...
      (while (< i num)
	(if (= i current-index)
	    (insert (propertize (format "%s\n" (cdr (assoc 'name (elt current-playlist i)))) ' face '(:foreground "Green")))
	  (insert-text-button (format "%s\n" (cdr (assoc 'name (elt current-playlist i)))) 'action (lambda(x)(play-with-index (button-get x 'index))) 'index i)
	  )
	(setq i (+ i 1))
	)
